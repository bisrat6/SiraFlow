...
// SECURITY: In production, store ARIFPAY_API_KEY in a secure vault (e.g., AWS Secrets Manager)
const ARIFPAY_API_URL = process.env.ARIFPAY_BASE_URL || "https://gateway.arifpay.net/api/";
...
const createB2CSession = async ({ amount, phoneNumber, reference, callbackUrl, merchantKey }) => {
  try {
    ...
    console.log(`[B2C Session] Creating session with merchant key: ${maskSensitiveData(merchantKey)}`);
    
    const response = await axios.post(sessionEndpoint, payload, { headers });
...
const executeB2CTransfer = async ({ sessionId, phoneNumber, merchantKey }) => {
  try {
    ...
    console.log(`[B2C Transfer] Executing transfer with merchant key: ${maskSensitiveData(merchantKey)}`);
    
    const response = await axios.post(transferEndpoint, payload, { headers });
...
const initiateTelebirrPayout = async (paymentId, merchantKey) => {
  try {
    // Validate merchant key is provided and not pending setup
    if (!merchantKey || merchantKey === 'pending_setup') {
      throw new Error("Valid merchant key is required for this transaction. Please configure your merchant key in company settings.");
    }
    ...
    if (process.env.ARIFPAY_DRY_RUN === "true") {
      ...
      console.log(`[B2C Dry Run] Payment ${paymentId} simulated with merchant key: ${maskSensitiveData(merchantKey)}`);
      return {
    ...
    // SECURITY: Validate merchant key format (basic validation)
    if (typeof merchantKey !== 'string' || merchantKey.length < 5) {
    ...
    console.log(`[B2C] Step 1: Creating session for payment ${paymentId} with merchant key: ${maskSensitiveData(merchantKey)}`);
    ...
    console.log(`[B2C] Step 2: Executing transfer for session ${sessionResult.sessionId} with merchant key: ${maskSensitiveData(merchantKey)}`);
    ...
    console.log(`[B2C] Transfer initiated successfully for payment ${paymentId} with merchant key: ${maskSensitiveData(merchantKey)}`);
...
const processBulkPayments = async (paymentIds, merchantKey) => {
  try {
    ...
    console.log(`[B2C Bulk] Processing ${paymentIds.length} payments with merchant key: ${maskSensitiveData(merchantKey)}`);
...
const retryFailedPayments = async (companyId, merchantKey) => {
  try {
    ...
    console.log(`[B2C Retry] Found ${failedPayments.length} failed payments to retry with merchant key: ${maskSensitiveData(merchantKey)}`);
